---
title: "Neural Network on the RBC Model"
format: html
---

The goal is to let a neural network learn (interpolate) the RBC model over a grid of parameter values. By doing this, we can simulate the model over a range of parameters without retraining the network.

## Setup

Both the benchmark and the neural network are based on the Euler equation. The benchmark uses time-iteration solved with bisection. The neural network uses Euler residual + out-of-bound capital penalty in the loss function.

The set of equations that describes the model is:
$$\begin{align}
 y &= A k^\alpha\\
 R &= \alpha A k^{\alpha-1} + 1 - \delta\\
 u(c) &= \frac{c^{1-\gamma}}{1-\gamma}\\
 u'(c) &= c^{-\gamma}\\
 u'(c) &= \beta E\left[u'(c'(k', A')) R(k', A')\right] \\
 k' &= A k^\alpha + (1 - \delta) k - c\\
 \log A' &= \rho \log A + \sigma_{\text{eps}} \varepsilon
\end{align}$$

See more details in the [README](../README.md).
...
println("Training bounds (box the NN was trained over):")
println("  k / k_ss:  ", training_bounds.k_bounds)
println("  log A:     ±", training_bounds.A_sigma_mult, " stationary std devs")
println("  alpha:     ", training_bounds.alpha_bounds)
println("  beta:      ", training_bounds.beta_bounds)
println("  delta:     ", training_bounds.delta_bounds)
println("  rho:       ", training_bounds.rho_bounds)
println("  gamma:     ", training_bounds.gamma_bounds)
println("  sigma_eps: ", training_bounds.sigma_eps_bounds)
